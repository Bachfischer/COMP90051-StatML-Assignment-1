...
def get_roc_score(edges_pos, edges_neg, emb):
    ...
    # perform predictions on statml test set
    import csv
    import pandas as pd
    import os

    # new instances where we do not know the answer
    test_csv = pd.read_csv('../data/test-public.txt',sep='\s+')
    test_pairs = test_csv.values.tolist()
    
    nodeslist = pd.read_csv('./nodeslist.csv',sep=',', names=['ID'])
    nodes = nodeslist['ID'].values.tolist()
    
    preds_test_set = []
    for connection in test_pairs:
        # Link Prediction on positive pairs
        try:
            index_source = nodes.index(connection[1])
            index_target = nodes.index(connection[2])
            preds_test_set.append(sigmoid(emb[index_source,:].dot(emb[index_target,:].T)))
            
        except ValueError:
            print("NOK")
            preds_test_set.append(0)

    with open('output.csv', mode='w') as file:
        file.write("Id,Predicted" + os.linesep)
        i = 0
        for pred in preds_test_set:
            content = str(i+1) + "," + str(preds_test_set[i])
